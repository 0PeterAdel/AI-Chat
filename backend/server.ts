...
import { fetch } from 'undici'; // استخدام undici كما طلبت
...
const constructOrganizerPrompt = (currentQuestion: string): string => {
    ...
    // نحصل على تاريخ اليوم بتنسيق YYYY-MM-DD ليكون مرجعًا دقيقًا للنموذج
...
# البيانات المتاحة لك:
لديك قائمة من الفعاليات. كل فعالية تحتوي على:
- \`title\`: اسم الفعالية.
- \`schedule\`: جدول زمني يحتوي على تاريخ ووقت الفعالية.
- \`categories\`: تصنيفات الفعالية (مثل: المعارض العلمية).
- \`targetGroups\`: الفئات المستهدفة (مثل: teacher, student).
- \`gender\`: الجنس المستهدف (all, boy, girl).

# قواعد صارمة لا يمكن تجاوزها:
1.  **الالتزام بالبيانات:** لا تقدم أي معلومات غير موجودة في قائمة الفعاليات. لا تخترع أو تخمن تفاصيل إضافية.
2.  **خارج النطاق:** إذا كان سؤال المستخدم لا يتعلق بالفعاليات (مثل: "ما هي عاصمة مصر؟" أو "اكتب لي قصيدة")، أجب بلباقة: "عفواً، أنا متخصص فقط في الإجابة على الأسئلة المتعلقة بالفعاليات والجداول الزمنية. هل لديك أي سؤال حول الفعاليات؟"
3.  **الفعاليات غير الموجودة:** إذا سأل المستخدم عن فعالية غير موجودة في قائمتك، أجب: "لم أجد فعالية بهذا الاسم في الجدول الحالي. هل تود البحث عن شيء آخر؟"

# كيفية الإجابة:
- **الفعاليات القادمة:** عند السؤال عن "أقرب فعالية" أو "الفعالية القادمة"، استخدم تاريخ اليوم (${today}) كمرجع لتحديد الفعالية التالية في الجدول.
- **التنسيق:** عند عرض قائمة من الفعاليات، استخدم قائمة نقطية (Markdown) لتكون الإجابة واضحة ومنظمة. مثال:
  - **اسم الفعالية:** (التاريخ) - (الوقت)
- **التفاصيل:** عند السؤال عن تفاصيل فعالية معينة، اذكر كل المعلومات المتاحة لك عنها (الاسم، التاريخ، الوقت، الفئات، الجمهور المستهدف).
...
app.post('/api/chat', async (req: Request, res: Response) => {
    try {
        ...
        // استخدام Optional Chaining للتأكد من وجود المسار بأمان
